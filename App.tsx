...
const DEFAULT_PROJECT: Project = {
  id: 'proj-001', name: '', clientAddress: '', city: '', postalCode: '', altitude: 0, windZone: WindZone.ZONE_1, distanceToPanel: 10, acCableSectionMm2: null,
...
function App() {
  ...
  useEffect(() => {
    ...
    if (totalPowerW > 0) {
        ...
        const isThreePhase = project.inverterConfig.phase === 'Tri';
				...
				const acCable = calculateAcCableSection(totalPowerW, project.distanceToPanel, cableDB as any, isThreePhase, project.acCableSectionMm2);
  ...
  const acCableMaterial = materials.find(
    (m) => m.description.includes('CABLE R2V') && (m.description.includes('3G') || m.description.includes('5G'))
  );

  // --- Liaison AC (tableau principal) : Auto vs Section forcée ---
  // 1) Section "Auto" (déduite du matériel proposé / calcul automatique)
  let autoAcCableSection = 2.5;
  if (acCableMaterial) {
    const desc = acCableMaterial.description;
    const match = desc.match(/[35]G(\d+\.?\d*)/);
    if (match) autoAcCableSection = parseFloat(match[1]);
  }

  // 2) Section effectivement utilisée pour les calculs (si l'utilisateur force, on respecte)
  const effectiveAcCableSection = project.acCableSectionMm2 ?? autoAcCableSection;

  // 3) Chute de tension recalculée avec la section effective
  ...
  // 4) Contrôle "protection vs section" (règle terrain conservatrice)
  ...
  // Valeur utilisée dans l'UI "Disjoncteur mini" (cohérente avec le PDF)
  const acBreakerMinA = recommendedBreakerA;
  const maxBreakerForSection: Record<number, number> = {
    2.5: 20,
    6: 32,
    10: 40,
    16: 63,
  ...
  const acProtectionTooHigh =
    recommendedBreakerA > (maxBreakerForSection[effectiveAcCableSection] ?? 0);
                        ...
                        <div className="mt-3 space-y-3">
                                ...
                                <div className="bg-slate-50 p-2 rounded-lg border border-slate-200">
                                    ...
                                    <div className="space-y-2">
                                        {Array.from(new Set(project.inverterConfig.configuredStrings.map(s => s.mpptIndex))).sort((a,b)=>a-b).map((mpptIndex) => {
                                            const run = (project.inverterConfig.dcCablingRuns || []).find(r => r.mpptIndex === mpptIndex) || { mpptIndex, lengthM: 0, sectionMm2: 6 };
                                            return (
                                                <div key={mpptIndex} className="flex items-center gap-2 bg-white rounded-lg border border-slate-200 p-2">
                                                        ...
                                                        <div className="grid grid-cols-2 gap-2">
                                                                ...
                                                                <input
                                                                    ...
                                                                    onChange={(e) => {
                                                                        ...
                                                                        setProject(prev => {
                                                                            ...
                                                                            runs.push({ mpptIndex, lengthM, sectionMm2: run.sectionMm2 || 6 });
                                                            ...
                                                            <label className="text-[9px] font-bold text-slate-400">
                                                                ...
                                                                <select
                                                                    ...
                                                                    onChange={(e) => {
                                                                        const sectionMm2 = parseFloat(e.target.value) || 6;
                                                                        setProject(prev => {
                                                                            const runs = [...(prev.inverterConfig.dcCablingRuns || [])].filter(r => r.mpptIndex !== mpptIndex);
                                                                            runs.push({ mpptIndex, lengthM: run.lengthM || 0, sectionMm2 });
                                                                            return { ...prev, inverterConfig: { ...prev.inverterConfig, dcCablingRuns: runs } };
                                                                        });
                                                                    }}
                            ...
                            {acProtectionTooHigh && (
                              <div className="mt-1 text-[10px] font-black text-red-700">
                                ⚠ Protection trop élevée pour {effectiveAcCableSection}mm²
                              </div>
                            )}
                            {!acProtectionTooHigh && project.acCableSectionMm2 && effectiveAcCableSection !== autoAcCableSection && (
                              <div className="mt-1 text-[10px] font-black text-slate-500">
                                Auto: {autoAcCableSection}mm² ({autoAcVoltageDropPercent.toFixed(2)}%)
                              </div>
                            )}
                                <div className="text-[8px] text-slate-400 font-bold italic mt-1">
                                    Coffret AC → point de raccordement
                                </div>
                            </div>
